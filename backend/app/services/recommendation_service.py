...
RECOMMEND_PROMPT = """你是一个智能学习推荐助手。根据用户的SQL练习表现数据，推荐最需要复习或学习的知识内容。

分析要点：
1. 找出得分低的题目对应的章节和知识点
2. 找出多次提交仍未正确的题目
3. 根据错误模式推断用户薄弱的知识领域
4. 推荐具体的知识树节点，帮助用户针对性提升
...
class RecommendationService:
    ...
    async def get_recommendations(self, db: AsyncSession) -> list[dict]:
        ...
        prompt_data = self._build_prompt_data(stats, knowledge_map)
        ...
        try:
            ...
            return self._fallback_recommendations(stats, knowledge_map)
    ...
    def _build_prompt_data(self, stats: dict, knowledge_map: list[dict]) -> str:
        knowledge_summary = {}
...
{related_text if related_text else '  无同章节题目'}"""
    ...
    def _fallback_recommendations(self, stats: dict, knowledge_map: list[dict]) -> list[dict]:
        weak_chapters = set()
        ...
        recommendations = []
        for k in knowledge_map:
            if k["chapter"] in weak_chapters:
                recommendations.append({
                    **k,
                    "reason": f"该章节练习表现较弱，建议重点复习",
                })
